...
const buildings = computed(() =>
    Array.from(new Set(resultArr.value.map((course) => course.description)))
)
...
const refresh = (val) => {
  // MOCK data，后端开启时候代替
  resultArr.value = courseStore.allCourse().filter(
      (course) => course.description.toUpperCase().includes(val.toUpperCase()) || course.description === val.toUpperCase()
  )
  // 初始化再次查询时候的变量值
  selectedType.value = ''
  selectedWeek.value = ''
...
const onTypeChange = (item) => {
  const filteredWeeks = Array.from(
      new Set(
          resultArr.value
              .filter((course) => course.type === item && course.description === selectedBuilding.value)
...
const onBuildingChange = (item) => {
  const types = Array.from(
      new Set(
          resultArr.value.filter((course) => course.description === item).map((course) => course.type)
      )
  )
  ...
  const filteredDayStartTimes = resultArr.value
      .filter(
          (course) =>
              ...
              course.week === item &&
              course.description === selectedBuilding.value
      )
      .map((course) => ({
